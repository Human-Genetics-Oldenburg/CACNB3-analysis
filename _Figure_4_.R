...
#
cairo_pdf(
  file = paste0(today, "_Ca_KCl_resting_fc.pdf"),
  width = 3,
  height = 4
)
par(
...
#
cairo_pdf(
  file = paste0(today, "_Ca_KCl_amplitude_fc.pdf"),
  width = 3,
  height = 4
)
par(
...
#
cairo_pdf(
  file = paste0(today, "_Ca_KCl_volume_fc.pdf"),
  width = 3,
  height = 4
)
par(
...
llimit <- 5
cairo_pdf(
  file = paste0(today, "_Ca_KCl_curves.pdf"),
  width = 5,
  height = 4
)
par(
...
p_wt <- which(peak_vol[, "WT", ] > llimit)
p_wt_m <- mod(p_wt, MAX_ROIs)
p_wt_i <- idivide(p_wt, MAX_ROIs) + 1
plot(
  STATIME:ENDTIME * 2,
  rowMeans(data_clean[, p_wt_m, "WT", p_wt_i], na.rm = T)[STATIME:ENDTIME],
  xlab = "time (s)",
  ylab = "F340/F380",
...
p_mut <- which(peak_vol[, "MUT", ] > llimit)
p_mut_m <- mod(p_mut, MAX_ROIs)
p_mut_i <- idivide(p_mut, MAX_ROIs) + 1
points(
  STATIME:ENDTIME * 2,
  rowMeans(data_clean[, p_mut_m, "MUT", p_mut_i], na.rm = T)[STATIME:ENDTIME],
  pch = ".",
  type = "l",
  col = colmut,
  lwd = 3
...
p_mock <- which(peak_vol[, "MOCK", ] > llimit)
p_mock_m <- mod(p_mock, MAX_ROIs)
p_mock_i <- idivide(p_mock, MAX_ROIs) + 1
points(
  STATIME:ENDTIME * 2,
  rowMeans(data_clean[, p_mock_m, "MOCK", p_mock_i], na.rm = T)[STATIME:ENDTIME],
  pch = ".",
  type = "l",
  col = colmock,
  lwd = 3
...
legend(
  ...
  paste0(
    ...
    c(length(p_mock), length(p_wt), length(p_mut)),
    rep(paste0("/", Ns, ")"), 3)
  ),
