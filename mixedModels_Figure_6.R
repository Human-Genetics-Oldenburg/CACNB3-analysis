...
#
cairo_pdf(
  file = paste0(today, "_Ca_ATP_resting_nMM.pdf"),
  width = 3,
  height = 4
)
par(
...
#
cairo_pdf(
  file = paste0(today, "_Ca_ATP_amplitude_nMM.pdf"),
  width = 3,
  height = 4
)
par(
...
p <- 0.45
lines(c(bar[1, 1] + 0.05, bar[2, 1] - 0.05), c(p, p), lwd = 2)
text((bar[1, 1] + bar[2, 1]) / 2, p + 0.01, "n.s.", cex = 0.8)
lines(c(bar[2, 1] + 0.05, bar[3, 1] - 0.05), c(p, p), lwd = 2)
...
#
cairo_pdf(
  file = paste0(today, "_Ca_ATP_volume_nMM.pdf"),
  width = 3,
  height = 4
)
par(
...
p <- 12
lines(c(bar[1, 1], bar[3, 1]), c(p, p), lwd = 2)
text((bar[1, 1] + bar[3, 1]) / 2, p + 0.2, "*", cex = 1.2)
p <- 11.3
lines(c(bar[1, 1] + 0.1, bar[2, 1] - 0.1), c(p, p), lwd = 2)
text((bar[1, 1] + bar[2, 1]) / 2, p + 0.2, "n.s.", cex = 0.8)
lines(c(bar[2, 1] + 0.1, bar[3, 1] - 0.1), c(p, p), lwd = 2)
text((bar[2, 1] + bar[3, 1]) / 2, p + 0.2, "n.s.", cex = 0.8)
...
#
# collapse data onto experiments (summarize individual cell measurements)
#
data_aln <- apply(data_aligned, c(1, 3, 4), mean, na.rm=T)
curves <- data.frame(apply(data_aln[STATIME:ENDTIME,,], c(1,2), mean, na.rm=T))

#
# Figure 6A
#
cairo_pdf(
  file = paste0(today, "_Ca_ATP_curves_aln_n.pdf"),
  width = 5,
  height = 4
)
par(
